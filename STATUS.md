...
| Phase | 内容 | 状態 |
|---|---|---|
| **A** | 記録層(3)＋一本道(1) | ✅ 実装・公開済（GAS再デプロイ済） |
| B | テストモード(4)＋重複管理(5) | ✅ 完了（v=56 testMode UI / v=59 破棄管理） |
| C | UI改善 8a/8b/8c/8d | ✅ 完了（8c v=56 / 8d v=58 acc別 / 8a8b v=60 拡大・画像位置） |
| D | 説明欄補助(6)＋カレンダー分離(7) | ✅ 完了（6 v=57 Qセーブ等 / 7 schedule v=22/23 ch分離） |
| E | PWA＋設定入出力＋オートフィル対策 8e/8f | ✅ 完了（8e v=56 / 8f v=61 PWA+設定IO） |
| （保留） | YouTube再生数(2)／Supabase本採用 | ⏸ スコープ外（ユーザー指示でやらない） |

> **2026-06-28 一括回収完了**：改修書 #5以降を安全な順（6→8d→5→8a/8b→8f＋7）で全実装し main へ公開（現在 v=61）。
> 7（カレンダーch分離）はサブエージェント実装→レビューで「実行フィールドのbody残置が他chへ漏れる」不具合を発見・修正済み（exec を唯一の正に）。
> 全テスト（6スイート・37+）PASS。PWA は Service Worker を入れず manifest のみ（?v= 更新方針との両立のため）。
