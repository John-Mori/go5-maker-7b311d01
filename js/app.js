...
(() => {
  ...
  // 録画の実尺(見た目の尺=DURATIONとは別)。iOS SafariのMediaRecorderは末尾を取りこぼすため、
  // 5.0秒ぴったりで止めると符号化尺が短く縮む(Chami報告2026-08-16 msg1538489722781831209)。
  // 見た目(ズーム/浮き上がり)はDURATION=5のまま=最終フレームを保持しつつ余白を録り、
  // 取りこぼしがあっても出来上がりが5秒台に着地するようにする(bgは録画中もloop再生=末尾も動画フレームが変化し続ける)。
  // ★実測の取りこぼし量は端末/iOS版で変わる: 当初は約1秒(REC=5→出来上がり4秒台)だったが、
  //   REC=6.2 で出来上がり「6秒」になったとChami報告(2026-08-16② msg1538516583654367255)＝現状の取りこぼしは約0.2秒。
  //   → 出来上がりを5秒台の中央(約5.4秒)へ寄せ、両側(4秒/6秒)の再発に対称な余白を持たせる値へ再校正。
  const REC_SECONDS = 5.6;
