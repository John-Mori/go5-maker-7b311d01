...
(function () {
  ...
  var TEMPLATES = {
    acc1: {
      ...
      price:    { src: 'assets/promo/tsukuyomi-price-base.png',
                  ...
                  // ★2026-08-18(3) 数字シート化後に数字を右へ(Chami『もう少し右』→『また5%右に』で0.558まで)→
                  //   『やりすぎたかも。ピッタリ真ん中に』(msg=1539139441866842142)で中央へ戻す。+5%は行き過ぎだった。
                  //   slot中心を0.558→0.508(x0.458→0.408)=札の中心線(作品/綴る/飾り罫の中心=x0.508)へ真ん中合わせ。
                  //   y/h/wは据置き。宵桜(acc2)は不変(C-035)。シート版(digitSheet)描画のまま座標だけ戻すので差し替え罠なし。
                  // ★2026-08-18(4)「10の数字をもう少し右に配置して」(Chami msg=1539217001350697030)。真ん中(中心0.508)から
                  //   控えめに右へ。前回の+5%(中心0.558)は『やりすぎ』だったので、その半分未満の+2%へ=x0.408→0.428(中心0.508→0.528)。
                  //   y/h/wは据置き・digitSheet描画のまま座標のみ。宵桜(acc2)は不変(C-035)。
                  slot: { x: 0.428, y: 0.503, w: 0.20, h: 0.103 } }
    },
    acc2: {
  ...
  // ★2026-08-28 Chami指示で新規動画/リセット時の既定を78%(0.78)へ変更。保存済みの手動調整値は維持する。
  var DEFAULT_SCALE = 0.78;
  // ★99円以下(二桁円セール=¥価格ラベル)は札を既定の90%サイズで出す(Chami依頼2026-08-11②)。
  //   0.78×0.9=0.702→0.70(UI表記70%)。二桁円の¥札は実額主役で大きく見えるため既定より一段小さく。
