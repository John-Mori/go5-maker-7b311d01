...
(function () {
  ...
  function fmtDelta_(d, tsMs, hasWork, prePost) {
    ...
    function cell(v, allowDash) {
      ...
      // 投稿前(YouTube未公開=投稿予定)/「今日投稿の昨日」だけは元々データが無い正当な空欄=– を出す(Chami指示2026-07-28/2026-07-30)。
      if (prePost || allowDash) return '–';
      // 投稿したて(36h以内)でまだスナップが取れていないだけ=欠損ではないので ⚠ でも – でもなく「未」(=未計測・毎時の自動記録で数値に変わる。Chami依頼2026-08-03)。
      //   ★freshPost を pending より先に見る。投稿直後はサーバーがこの動画をまだ知らず delta が丸ごと未着(pending)になるため、
      //     ここを pending 優先にすると投稿したての全セルが「–」に落ち「未」が永遠に出ない=Chami設定の無視(2026-08-13「投稿したてで取得がまだの場合は未にしてって言ったやろ、設定消すな」)。
      if (freshPost) return '<span title="投稿したてで、まだ計測記録が取れていません(毎時の自動記録で数値に変わります)。⚠は追跡開始前の期間か取得失敗を表し、これとは別です">未</span>';
      // 投稿済みだが古い/一過性のロード中で delta が未着=–(SWRでこの後 refreshDeltas_ が数値へ差し替える)。
      if (pending) return '–';
  ...
  // 発売日(YYYY-MM-DD…)→現在の作品状態。判定は core/movie-attrs-core.js に一本化(2026-08-13・C-038)。core未ロード時は同ロジックで代替。
