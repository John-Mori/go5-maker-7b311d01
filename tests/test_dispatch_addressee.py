...
def main():
    ...
    # ★経由便の助言文(2026-08-23 研究室HQ指摘)= 3階梯を正しく使うたびに
    #   実行不能な助言(「見出しを直せ/その部門へも出せ」)が鳴るのを止める。**鳴ること自体は残す。**
    w9 = addressee_warning(
        "【研究室HQ → イージス研究室(部門長)経由 → 改善提案部門(トトリ)】裁定した\n本文",
        ["aegis-gl"])
    check("W-9 経由便でも鳴る(手渡し待ちの部門を沈ませない)", "改善提案部門" in w9, w9)
    check("W-10 経由便の助言は『転送されたか確認しろ』へ替わる",
          "転送されたか" in w9 and "見出しを直すか" not in w9, w9)
    # ★must-fail= 経由と書いてあっても、**投函先が経由の左側に居なければ**ただの宛先ズレ扱い
    w11 = addressee_warning(
        "【研究室HQ → イージス研究室(部門長)経由 → 改善提案部門(トトリ)】裁定した\n本文",
        ["hr-room"])
    check("W-11 経由の相手ではない所へ出した時は従来の助言(見出しを直せ)のまま",
          "見出しを直すか" in w11 and "転送されたか" not in w11, w11)
    # ★must-fail= 「経由」の無い普通の見出しは、経由文言へ倒れない
    check("W-12 経由の無い見出しでは従来の助言のまま",
          "見出しを直すか" in w1 and "転送されたか" not in w1, w1)
