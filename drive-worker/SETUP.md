...
> **破壊面の不変条件(このツールがすること・しないこと)**
> 行うのは「フォルダの新規作成」「ファイルの新規アップロード」「参照(読み取り)」＋
> **「フォルダのゴミ箱送り(trashed=true)ただ1種」** だけです。
> **完全削除・改名・移動はしません**(files.delete / name更新 / parents更新のAPIは不在)。
> ゴミ箱送りは `ALLOW_OVERWRITE="1"` かつフロントが `overwrite=1` を送った時だけ発動し、
> **窓内(作成30日以内)の同名フォルダを「新規保存の"後"に」** 送ります(先に上げてから消す＝喪失ゼロ・30日間復元可)。
> 上書きが無効(既定/`_N`連番)の時は既存に一切触れません。詳細＝手順7「同題名の上書き」。
...
### 同題名の上書き(`ALLOW_OVERWRITE`)
`ALLOW_OVERWRITE = "1"`(`wrangler.toml` の `[vars]`)が有効な時だけ、フロントが `overwrite=1` を送った保存で、
**窓内(作成30日以内・DriveのcreatedTimeが正)の同名フォルダを「新規保存の"後"に」ゴミ箱送り**する(先に上げてから消す＝
喪失ゼロ／30日間Drive側で復元可)。空/未設定/他値なら従来どおり `_2, _3…` の別名で非破壊に新規作成する二段ロック。
無効化したい時はこの行を消して(または `"0"` にして)再デプロイするだけでよい。完全削除・改名・移動のAPIは実装していない。
...
4. もう一度同じタイトルで作成 → `ALLOW_OVERWRITE="1"` なら **同名フォルダが上書き**され、旧フォルダが Drive の
   ゴミ箱へ移る(新規保存の"後"に送る＝一瞬2つ見えてから旧が消える)ことを確認。`ALLOW_OVERWRITE` 未設定なら
   従来どおり `タイトル_2/` が新規作成される(既存に触れない)。
5. 問題なければ `FOLDER_ID_ACC1` を本番(月詠み)に戻して再デプロイ。

## 安全確認(破壊面が「ゴミ箱送り1種」に限られることの証明)

このフォルダで：
```bash
grep -nE "files.delete|removeParents|addParents" src/index.js   # ← 何も出なければOK(完全削除・移動が無い)
grep -n "\"name\":" src/index.js                                 # ← 改名(name更新のPATCH)が無いこと
grep -n "trashed" src/index.js                                   # ← 破壊操作は trashed=true の PATCH ただ1箇所(trashFolderGuarded)だけ
```
- 完全削除(`files.delete`)・改名・移動のAPIは**1つも呼びません**。
- 唯一の破壊操作は `trashFolderGuarded` の `trashed:true`(ゴミ箱送り・30日復元可)で、`ALLOW_OVERWRITE="1"`＆
  `overwrite=1`＆窓内同名フォルダの三条件が揃った時だけ、trash直前に親直下・フォルダ・題名一致・窓内を再検証して実行します。
