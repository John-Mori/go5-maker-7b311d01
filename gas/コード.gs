...
function doGet(e) {
  ...
  // 提案決定ページ(改修α): 決定シート「提案決定」を外から読む口。(読み取り専用・追加のみ・書き込みゼロ)
  //   イージス研究室GL→改修α発注(2026-08-23)。エコー経路B=PC側ポーラーがこの口で新しい決定行を拾い、軍議へdispatchする。
  //   ?action=teian_decisions&since_row=<N> → 行番号 > since_row の行だけを返す(冪等はこの行番号1本で持つ)。
  //   since_row 省略時は 1(=ヘッダの次から)。1回の上限は50行(残りは次周期)。
  //   シート未作成時は ok:true, lastRow:0, rows:[] を返す=fail-open(決定0件の間に監視が警報を上げない)。
  //   ヘッダは実物のシート1行目から読んで返す=将来 teianDecide_ の列が増えても監視側が壊れない。
  if (p.action === 'teian_decisions') {
    try {
      var tsh = openSS_().getSheetByName('提案決定');
      if (!tsh) return jsonOut_({ ok: true, headers: [], lastRow: 0, rows: [] });
      var tLast = tsh.getLastRow(), tCols = tsh.getLastColumn();
      if (tLast < 1 || tCols < 1) return jsonOut_({ ok: true, headers: [], lastRow: 0, rows: [] });
      // Sheetsは日時文字列をDate値へ勝手に格上げする(getDisplayValuesでもlocale長形式 "Sun Aug 23 2026…" になる)。
      //   読み取り側で Date だけ yyyy-MM-dd HH:mm:ss へ整形=既存行/新規行どちらも軍議エコーが綺麗。他はString()。
      var tfmt = function (v) { return (v instanceof Date) ? Utilities.formatDate(v, 'Asia/Tokyo', 'yyyy-MM-dd HH:mm:ss') : String(v); };
      var tHead = tsh.getRange(1, 1, 1, tCols).getValues()[0].map(tfmt);
      var sinceRow = Math.max(parseInt(p.since_row || '1', 10) || 1, 1);
      var startRow = Math.max(sinceRow + 1, 2); // 1行目(ヘッダ)は返さない
      var trows = [];
      if (tLast >= startRow) {
        var take = Math.min(50, tLast - startRow + 1);
        var tvals = tsh.getRange(startRow, 1, take, tCols).getValues();
        for (var ti = 0; ti < tvals.length; ti++) {
          trows.push({ row: startRow + ti, values: tvals[ti].map(tfmt) });
        }
      }
      return jsonOut_({ ok: true, headers: tHead, lastRow: tLast, rows: trows });
    } catch (err) { return jsonOut_({ ok: false, error: String(err) }); }
  }
  ...
  if (p.action === 'comp_status') { try { return jsonOut_(compStatusToday_()); } catch (err) { return jsonOut_({ ok: false, found: false, error: String(err) }); } }  // 競合_日次ステータスの本日行(silent green封じの読み手・PC側日次赤判定用・読み取り専用)
...
function doPost(e) {
  ...
  try {
    ...
    // 提案決定ページ(改修α): 「この作品・この④コメントで決定」を記録し、軍議へエコー(webhook設定時)。
    if (body.op === 'teian_decide') return teianDecide_(body);
    ...
    var r = writeRecord_(body.channel || 'acc1', {
      ...
      // 投稿先はフロントの明示値を最優先。旧Bluesky経路はplatform未送信でもpostUri(at://)が
      // 必ず付くため、そこだけbskyへ安全に補完する(Xを推測で上書きしない)。
      platform: (body.platform === 'x' || body.platform === 'bsky') ? body.platform : (/^at:\/\//.test(String(body.postUri || '')) ? 'bsky' : ''),
...
// 提案決定ページの決定を記録し、軍議へエコー。
//   ・シート「提案決定」へ1行追記(冪等ヘッダ・durable)=webhook未設定でも決定は必ず残る。
//   ・prop_('GUNJI_WEBHOOK') があればDiscord webhookへembed送信(fail-open=送信失敗でも記録は成功)。
//   ・破壊面ゼロ(追記のみ・既存シート/列に触れない)。
function teianDecide_(body) {
  try {
    var comment = String(body.comment || '').trim();
    var title = String(body.title || '').trim();
    if (!comment) return jsonOut_({ ok: false, error: 'comment_required' });
    var ch = String(body.channel || 'acc1');
    var chName = sheetName_(ch); // 月詠み / 宵桜艶帖
    var ss = openSS_();
    var sh = ss.getSheetByName('提案決定');
    var HEAD = ['決定日時', '候補日', '候補ID', '作品cid', 'プラットフォーム', 'チャンネル', '作品タイトル', '④コメント', '経路'];
    if (!sh) { sh = ss.insertSheet('提案決定'); sh.appendRow(HEAD); }
    else if (sh.getLastRow() === 0) { sh.appendRow(HEAD); }
    var nowJst = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy-MM-dd HH:mm:ss');
    sh.appendRow([nowJst, String(body.date || ''), String(body.id || ''), String(body.cid || ''),
      String(body.platform || ''), chName + '(' + ch + ')', title, comment, String(body.source || 'teian-page')]);
    var row = sh.getLastRow();
    // 軍議へエコー(webhook設定時のみ・失敗しても記録は成功のまま返す)
    var discord = 'skipped_no_webhook';
    var hook = prop_('GUNJI_WEBHOOK');
    if (hook) {
      try {
        var content = '【提案決定】' + chName + ' へ「' + title + '」で投稿決定。\n④コメント= ' + comment + '\n(cid=' + String(body.cid || '') + ' / ' + nowJst + ' JST)';
        var res = UrlFetchApp.fetch(hook, {
          method: 'post', contentType: 'application/json',
          payload: JSON.stringify({ content: content.slice(0, 1900) }), muteHttpExceptions: true
        });
        var code = res.getResponseCode();
        discord = (code >= 200 && code < 300) ? 'sent' : ('error:http_' + code);
      } catch (we) { discord = 'error:' + String(we); }
    }
    return jsonOut_({ ok: true, row: row, channel: ch, discord: discord });
  } catch (err) {
    return jsonOut_({ ok: false, error: String(err) });
  }
}
...
function writeRecord_(channel, f) {
  ...
  // Bulk sync may come from a stale device. Read the two mutable authoritative cells once,
  // then only fill server blanks; explicit per-item edits still use the normal overwrite path.
  var preserveMutableExisting = !!f.preserveMutableExisting && !isNewRow;
  var existingPlatform = '', existingWorkShort = '';
  if (preserveMutableExisting) {
    var mutableCols = [map['投稿先'], map['作品短縮URL']].filter(function (c) { return !!c; });
    if (mutableCols.length) {
      var mutableMin = Math.min.apply(null, mutableCols), mutableMax = Math.max.apply(null, mutableCols);
      var mutableRow = sh.getRange(target, mutableMin, 1, mutableMax - mutableMin + 1).getValues()[0];
      if (map['投稿先']) existingPlatform = String(mutableRow[map['投稿先'] - mutableMin] || '');
      if (map['作品短縮URL']) existingWorkShort = String(mutableRow[map['作品短縮URL'] - mutableMin] || '');
    }
  }
  ...
  if (!(preserveMutableExisting && existingPlatform !== '')) {
    putIf('投稿先', (f.platform === 'x' || f.platform === 'bsky') ? f.platform : ''); // Bulk sync fills blanks only; explicit edit still overwrites.
  }
  putIf('短縮URL', shortUrl);                                   // r2＝計測用(codeFromShort_対象・r2以外は入れない)
  if (!(preserveMutableExisting && existingWorkShort !== '')) {
    if (f.workShortClear) put('作品短縮URL', '');                // Explicit edit can clear; bulk sync cannot erase a non-empty server value.
    else putIf('作品短縮URL', f.workShortUrl || '');             // Bulk sync only fills a server blank.
  }
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    try {
      writeRecord_(channel, {
        ...
        workShortClear: it.workShortClear === true || it.workShortClear === 'true', // String "false" is not a clear command.
        // 一括同期でも手動指定を落とさない。旧Bluesky行はat:// postUriだけを根拠にbsky補完する。
        platform: (it.platform === 'x' || it.platform === 'bsky') ? it.platform : (/^at:\/\//.test(String(it.postUri || '')) ? 'bsky' : ''),
        ...
        preserveMutableExisting: true   // Stale bulk sync must not roll back platform/work-short truth.
      });
      n++;
    } catch (e) {}
  }
...
function setupTrigger() {
  ScriptApp.getProjectTriggers().forEach(function (t) {
    ...
    if (f === 'refreshClicks' || f === 'refreshEngagement' || f === 'snapshotStats' ||
        f === 'runCompetitorDaily' || f === 'runCompetitorDailyWatched' || f === 'runCompetitorDiscovery') ScriptApp.deleteTrigger(t);
  });
  ...
  //   ★自動経路は runCompetitorDailyWatched(silent green封じの入口)を呼ぶ=戻りok:falseを捨てず
  //     競合_日次ステータスへ毎回記録＋失敗時はDiscordへbest-effort通知(モドリッチ依頼2026-08-23/B)。
  ScriptApp.newTrigger('runCompetitorDailyWatched').timeBased().everyDays(1).atHour(4).create();
    ...
    try {
      ...
      try {
        ...
        writeRecord_(ch, {
          ...
          platform: 'bsky', // GAS無人予約はこの関数自身がBlueskyへ投稿した確定経路
