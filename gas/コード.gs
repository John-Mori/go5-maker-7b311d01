...
function snapshotStats() {
  ...
  var RUN_T0 = Date.now();
  var BUDGET_DATEFIX_MS = 120000; // 日時バックフィルは最大2分(残り時間をクリック取得へ残す)
  var BUDGET_TOTAL_MS = 240000;   // クリック取得は通算4分で打ち切り(6分制限まで余裕を残す)
  ...
  recs.concat(tpRecs).forEach(function (r) {
    if (Date.now() - RUN_T0 > BUDGET_TOTAL_MS) return; // ★予算超過後はクリック取得を打ち切る(未取得コードは次回続行)。views/ピークは下で必ず書く=関数は必ず完走する
