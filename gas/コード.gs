...
function doGet(e) {
  ...
  if (p.action === 'teian_decisions') {
    try {
      ...
      // getDisplayValues=セルの表示文字列そのまま(Sheetsが日時文字列をDate値へ勝手に格上げしても、表示=teianDecide_が書いた yyyy-MM-dd HH:mm:ss のまま返る=軍議エコーが綺麗)
      var tHead = tsh.getRange(1, 1, 1, tCols).getDisplayValues()[0].map(function (v) { return String(v); });
      ...
      if (tLast >= startRow) {
        ...
        var tvals = tsh.getRange(startRow, 1, take, tCols).getDisplayValues();
        for (var ti = 0; ti < tvals.length; ti++) {
          trows.push({ row: startRow + ti, values: tvals[ti].map(function (v) { return String(v); }) });
        }
      }
