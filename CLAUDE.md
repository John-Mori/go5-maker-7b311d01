...
13. **スケジュール統合 Phase1（v=13）**：投稿スケジュール最適化システムを **📅カレンダータブ**として統合（`schedule/` に配置し **iframe** 埋め込み＝CSS/JS衝突回避・既存機能は回帰ゼロ）。同一オリジンで localStorage 共有。タブは 🎬動画作成／📅カレンダー／🦋投稿／🔗アフィリンク（🧪検証はカレンダー内）。設計書＝`5秒動画タイムスケジュール/docs/統合設計書_動画メーカー×スケジュール.md`。次：Phase2 一気通貫（スロット↔作成↔投稿の橋渡し）。
