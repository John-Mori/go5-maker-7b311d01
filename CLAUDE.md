...
> **巻の引き継ぎ**：開発チャットは巻(Vol.)で分かれる。現在 **Vol.7**（2026-07-14切替・live=`?v=331` / GAS=`2026-07-07A`）。新チャットは最初に **`引き継ぎ_Vol7.md`** を読むこと（前巻の詳細は `引き継ぎ_Vol6.md`）。ミス台帳は `インシデント.md`。★UI文言の括弧は必ず半角`()`（全角`（）`禁止）。
...
> **AI部門制・司令塔(2026-07-11導入・実装Go済)**：メインセッション=司令塔(モデルはFable 5→提供終了後Opus)。タスクは `.claude/agents/` の7部門(system-engineer/product-scout/copy-director/shorts-analyst/qa-reviewer/kaizen-analyst/learning-coach)へ、モデル(opus/sonnet/haiku)を**明示指定**して振り分ける。ルール正本=**`docs/departments/00_common/orchestration.md`**(必読・品質優先6段原則/Python積極利用[許可不要]/イベント購読表/権限規定)。Chamiの改善要求は D1 `go5_kaizen`(12表・稼働中) の improvement_requests へ、デプロイ/バンプは system_changes へ司令塔が記録。**改善は承認制**(提案→Chami承認→実装)。**品質を犠牲にしたトークン節約は禁止**(正確性>安全性>検証可能性>保守性>トークン効率>速度)。ロードマップ=`docs/設計・調査/AI組織_実装設計書_v3_統合ロードマップ.md`§10(S0-S2構築済)。
