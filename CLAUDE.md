...
### 変更履歴(開発の順番)
18. **Google Drive 自動保存**：★**保存タイミングはドラフトの「投稿完了」を押した時に一本化**(Chami指示2026-08-13。旧`video-created`即保存は廃止=作成時にはDriveへ書かない)。投稿完了(`stock.js` driveSaveForCompleted_)が生成動画＋元画像＋仕上がりプレビューを **Cloudflare Worker 経由**で `マイドライブ/AFI5秒動画/[チャンネル]/[動画名]/` へ1度に保存。動画blobは作成直後に`ensureVideoMirror_`でR2へ控え、投稿完了時に`resolveVideoBlob_`が手元IDBに無ければR2から取り寄せる=作成〜完了の間にiOSがblobを捨てても喪失しない。フロント＝`drive-upload.js`(チャンネルは作成イベントの`account`優先→`getCurrentAccount()`、未判定なら保存せずエラー。失敗しても投稿完了は成功のまま・リトライ可)。Worker＝`drive-worker/`(**破壊面の不変条件：フォルダ新規作成・ファイル新規アップロード・参照＋"フォルダのゴミ箱送り(trashed=true PATCH)ただ1種"のみ。完全削除/改名/移動APIは不在＝grep検証は"trashed"1語**)。★**同題名は上書き**(Chami指示2026-08-13)＝`overwrite=1`(フロント明示)かつ`env.ALLOW_OVERWRITE='1'`(サーバ側キルスイッチ)の二重ロック時だけ、窓内(作成30日以内・DriveのcreatedTimeが正)の同名フォルダを **新規保存の"後"に**ゴミ箱送り(先に上げてから消す=喪失ゼロ/30日復元可/trash直前に親直下・フォルダ・題名一致・窓内を再検証)。窓外/同名なし/フラグ無しは従来どおり `_2,_3…` 連番で非破壊。認証は本人OAuth refresh_token(個人Gmailのため Service Account 不可)を **Worker Secrets** に保管。スコープは `drive`(既存フォルダへ書くため／作成専用コードで運用。承認済)。Origin制限＋共有シークレット＋KV日次レート制限の多層防御。フォルダIDは env 固定(取り違え防止)。手順＝`drive-worker/SETUP.md`。秘密はフロント/repo/ログに出さない(SHARED_SECRETのみフロント可＝ソフト鍵)。
...
### ★現運用の投稿導線(2026-08-02 確定・以下 §9/§10 の前提)
- **中間はもう Bluesky ではなく X。** 現運用= `YouTube → 短縮URL(独自ドメイン 5mgl.com/yoz2.com) → X(アフィリンクも同独自ドメインで短縮) → FANZA`。Bluesky凍結騒ぎのため X へ切替済(2026-08-02)。
- **X投稿は手動運用。** このアプリはクライアントサイドで X へ直投稿できない(=下の §10 末尾「X はこの構成では不可」の通り。OAuth＋サーバー必須)。**したがって下の §9「Bluesky 投稿」はアプリに実装された機能の説明で、コードは残置しているが現在の投稿導線では使っていない**(§10 の Bluesky公開API エンゲージ取得・`bsky.app` 共有URL も Bluesky経路に紐づく=現運用では非経路)。※アプリ機能仕様として残す。
- **★X運用のP0作法=アフィリンクは投稿本文へ直ポストせず"返信"に置く**(本文直ポストで凍結の実例あり)。独自ドメイン短縮によるクリック計測(§10・link-worker 開封数)は投稿先が X でも成立する(短縮URLの開封を数えるため)。
...
- **FANZA直リンクを YouTube 説明欄・固定コメントに絶対に貼らない。** 正は現行の `YouTube → 短縮URL(独自ドメイン 5mgl.com/yoz2.com) → X 投稿 → FANZA` の2段クッション構造(★2026-08-02 中間を Bluesky→X へ切替=Bluesky凍結騒ぎのため運用判断で移行。旧記述「→短縮URL(r2/da.gd)→Bluesky投稿→」は退役)。生アフィリンクが出てよいのは X 投稿本文のみ。**★X運用のP0作法=アフィリンクは投稿本文へ直ポストせず"返信"に置く**(本文直ポストで凍結の実例あり)。
