...
> **巻の引き継ぎ**：開発チャットは巻(Vol.)で分かれる。現在 **Vol.7**(2026-07-14切替・live=`?v=331` / GAS=`2026-07-07A`)。新チャットは最初に **`引き継ぎ_Vol7.md`** を読むこと(前巻の詳細は `docs/引き継ぎ/引き継ぎ_Vol6.md`。旧巻はすべて `docs/引き継ぎ/` へ移動・2026-07-17)。ミス台帳は `インシデント.md`。★UI文言の括弧は必ず半角`()`(全角`()`禁止)。
