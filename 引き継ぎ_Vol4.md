...
| GAS_VERSION | ローカル=**`2026-07-05H`**（D-1）／**live=まだ`2026-07-05G`＝GAS未デプロイ**（下記⚠） |
| ブランチ / 作業ツリー | `main` |
...
- **⚠ D-1（記録取りこぼし回収）はコード完了だが GAS 未デプロイ**：`gas/コード.gs` を `2026-07-05H` に更新済（hashtags/リビルド元ID/タイトル文字数の記録・予約経由投稿への動画メタ中継）。フロント（bluesky.js の reserveUnattended meta送信）は反映済で**後方互換**（旧GASは未知フィールドを無視）。**要 `node scripts/deploy_gas.mjs`（本番反映＝Chami許可 or 手動実行）**→ 実行で列追加(migrate)＋トリガー再設定まで自動。反映後 `?ping=1` が `2026-07-05H` を返せばOK。
