...
- **S-3 メタデータ**：`buildTitle` にタグ数ガードレール（3〜5推奨・#Shorts推奨・`#ytTagWarn` に表示。※「15個超で全無効」は誤情報と判明→公式仕様=60個超で全無視・表示は上位3個のみ、に修正済 2026-07-05）。既定タグを `#Shorts #マンガ #漫画紹介 #anime` に。**非破壊**（ユーザーのタグは書き換えない）。2ch別文言は既存の per-account テンプレ（yt_tags__/bsky_text__）で構造的に対応済。
...
| S-3 | メタデータ整備：ハッシュタグ3〜5個(公式仕様=60個超で全無視・表示は上位3個)・題名にジャンル語・2ch別文言・投稿は夕方/金曜厚め | bluesky.js, schedule/ | 生成タグ3〜5・2ch別文言 | Sonnet |
