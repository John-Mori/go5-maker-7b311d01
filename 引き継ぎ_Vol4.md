...
- **✅ 戦略ラベル記録（v205・GAS `2026-07-05I`）**：動画作成タブに「🎯狙い（成約/集客）」「💬コメント型（①〜⑧）」セレクトを追加→記録シートに「目的」「コメント型」列を自動記録（即時/予約/無人予約/一括同期の全経路）。**投稿後は自動で未設定へ戻る**（field_も削除＝前作の値の残留防止。U-1と同思想）。背景＝`raw/戦略_画像選びとコメント.md`（**raw/はgit管理外・コンサル資料のため公開リポジトリ禁止**）の週次「勝ちパターン表（目的×型×カテゴリ）」を自動集計可能にするため。同戦略の裏取りで**「タグ15個超で全無効」は誤情報（公式=60個超で全無視・表示上位3個）と判明→v204で修正済み**。
- **ベースライン（Studio目視）＝Chami運用**（swipe率・フィード表示回数・年齢制限有無を記録）。改善書§9。

### ✅ 追加改修（Vol.4・改善書外の実運用対応）
- **戦略ドキュメント策定**：`raw/戦略_画像選びとコメント.md`（**raw/はgit管理外＝コンサル資料・公開リポジトリ禁止**）。okada勉強会レポート＋Web裏取り＋レッドチーム検証。画像選び(ハードフィルタ→止まる力スコア→目的タグ)・コメント8型×コマ相性表・**検証は目的別二系統**(集客=維持率分布/成約=クリック数)。→ この分析を回すため「🎯狙い/💬コメント型」記録機能を実装(v205・GAS I)。
- **INC-69: 候補画像をIndexedDBへ移行（v206）**：iOS Safariの localStorage 約5MB壁で候補画像の保存が「容量不足」で失敗していた問題を根本解決。`core/idb-store.js`(Go5Idb)新設＋`candidates.js`の refimg/bskyimg を IDB化（起動時ハイドレートで同期読み維持・旧localStorage画像は自動移行）。以後は端末の空き容量に応じて数百MB〜保存可能。
- **投稿編集モーダル改善（v207）**：保存直後にカードへサムネ即反映／貼り付けボタン白文字化／作品URLのプレフィル＋動画生成への確実な反映／空コメント転送時に3段目を空に上書き。
- **候補タブ改善バッチ1（v208）**：モーダル右上に月詠み/宵桜艶帖ボタン（投稿履歴のworkUrl→cid照合で**そのchで投稿済みなら「✓投稿済」アンバー表示**＝P0-3重複投稿の抑止・クリックでch切替。`bluesky.js`に`window.Go5PostedWorkUrls(acc)`公開）／削除ボタンを🗑️のみに／重複追加時「この作品は既に追加されています」。
- **複数画像対応＝バッチ2（v209）**：refimg のデータモデルを `{img}`→`{imgs:[...], img:先頭(旧互換)}` に拡張（読みは `refImgsOf_` で正規化＝旧データ無移行で動く）。**貼り付け/ファイル選択（multiple対応）は追加式**＝37ページ級の連続貼り付けOK。モーダルプレビューは‹›ボタン＋スワイプで切替・「複数あり n/m」表示・**「動画生成へ」は表示中の画像を採用**。消す＝表示中の1枚を削除。カードは先頭画像＋「複数あり ×N」バッジ・タップで全画像ズーム(スワイプ)。※Bluesky添付画像モーダル(bskyimg)は仕様上1枚のまま。
