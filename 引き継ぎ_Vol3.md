...
**`docs/設計・調査/改善書_Shorts攻略と全体改善ロードマップ.md`** に、①作りやすさ②データ分析③保守性④**Shortsフィード攻略(最重要)** の4本柱で Phase 0〜4 のタスク表（触るファイル・受け入れ条件・Opus/Sonnetモデル割当つき）を策定済み。着手前に改善書§1の**裁定4項目**（KPI=engagedViews推奨／尺5秒維持かAB／Data API監査申請／リファクタ先行度）をChamiに確認すること。Phase 0の筆頭は P0規約防衛（運用ルール明文化）と M-1共通コア3ファイル。即効打は S-1フック強化+完全ループ化（**t=0で前景が透明**という現行仕様の解消）と S-2 BGM追加（**現行は完全無音**）。既知バグ U-7=wizardのイベント不達（window購読 vs document発火）も改善書に収録。
