...
**`docs/設計・調査/改善書_Shorts攻略と全体改善ロードマップ.md`** に、①作りやすさ②データ分析③保守性④**Shortsフィード攻略(最重要)** の4本柱で Phase 0〜4 のタスク表（触るファイル・受け入れ条件・Opus/Sonnetモデル割当つき）を策定済み。**裁定4項目は決定済み（2026-07-05 Chami）**: KPI=**engagedViews**／尺=**5秒維持**(尺拡張AB不採用・ループ数で勝負)／Data API監査=**申請する**／リファクタ=**M-1をPhase 0で先行**。次にやるのは改善書§8 Phase 0: P0規約防衛の明文化・S-6・U-7（wizardイベント不達バグ修正=window購読vs document発火）・M-1共通コア3ファイル。その後 Phase 1 の即効打= S-1フック強化+完全ループ化（**t=0で前景が透明**の解消）と S-2 BGM追加（**現行は完全無音**・CC0限定）。
