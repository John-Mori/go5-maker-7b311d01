...
4. ★**コピー変数の集計でチャンネルを層別条件にしない(Chami裁定2026-07-18)**: ④コメントの口調・型・作品選定はチャンネル無関係(chの差はガワ=導入/ネーミングのみ)。よって「題名型/コメント型/タグ ×成績」の相関を見る時、月詠み/宵桜で分けて集計しない=両ch束ねて"再現性のある勝ちパターン"を抽出する。ch別に切ると成績データがノイズ化する(裁定の理由そのもの)。※acc接頭辞で混ぜない(項3)のは videoId の取り違え防止であって、コピー変数の層別とは別問題
5. 書き込み禁止(読み取り・分析・提案のみ)。秘密を出力しない
