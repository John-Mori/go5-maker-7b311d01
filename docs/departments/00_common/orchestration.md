...
0. **【最重要】作業ディレクトリ規約 (2026-07-15 INC・分類器障害で判明)**: **必ず `D:\SougouStartFolder\5SecMovieMaker` 直下でセッションを起動する**。外フォルダ(例 `D:\総合スタートファイル\動画・画像編集\5秒動画`)から開いて `cd` で5SecMovieMakerへ跨ぐと、**毎コマンドが分類器判定にかかり、分類器(claude-opus-4-8)障害時にBash/Edit(=書き込み)が全滅**して作業が「適用待ち」で止まる(2026-07-15に実害・多数タスク停止)。起動時に **`node -e "console.log(process.cwd())"` でcwdを確認**し、5SecMovieMaker直下でなければ**そこで止めてChamiへ「5SecMovieMaker直下で開き直して」と要請**する(cd跨ぎで続行しない)。ワンクリック起動=`起動_5SecMovieMaker.bat`(5SecMovieMaker固定・罠フォルダのlaunch.jsonから5SecMovieMaker/vol系サーバ定義は撤去済)。
...
2. **部門アクセス境界**: 各人格は自部門+明示許可された部門のみ入室(正=**`D:\SougouStartFolder\00_AI-HQ\departments\hr\personas\INDEX.md`**——★人事部門・ペルソナ台帳は2026-07-18にHQ直下へ移転(Chami指示・全PJ管轄化+公開repo機密防衛)。5SecMovieMaker内の旧`docs/departments/personas/`・`hr-room/`は退役=README_移転.mdのみ)。越境しない。見つけたら人事(アメス)が本来の部門へ差し戻す。
...
## 不変条件(全部門共通)
半角括弧 / 秘密を出力・コミットしない / ?v=一括バンプ / Worker/GASのデプロイは**承認不要**(★2026-07-21 Chami裁定「デプロイしていい、デプロイも特に承認は要らないんだけどね」。旧=Workerは要承認/GASは不要だったが、Workerも不要へ統一。★なお `link-worker/**` の push は GitHub Actions で**自動デプロイ**される=pushした時点で本番に出る) / KV dedup維持 / 成約は観測不可(追わない) / 品質優先6段原則(冒頭) / 改善は承認制(観測→仮説→提案→Chami承認→実装→効果測定) / **戦略・事業文書はGitHubに上げない**(Chami指定 2026-07-12: リポジトリに上げるのはプログラム等の必要物のみ。戦略・目標・優先度は `local/`〔gitignore済〕へ。AIはローカルを直接読むため公開不要) / **★Google Driveは許可されたクラウド(Chami許可2026-07-16「他のクラウドとは違って特別な場所」)**: `local/`の記録(機微=dreams/past/健康を含む)を**Driveへバックアップしてよい**。
...
4. ★**ミラーは通知を鳴らさない+スタンプを押す (2026-07-18 Chami指示・QA実装)**: `Chami(`で始まる名義はpersona_sendが**自動でサイレント送信**(SUPPRESS_NOTIFICATIONS=専用bot不要でChamiに通知が行かない)し、**msg=IDを表示**する。鳩/Gatewayはミラーへ**送信印を自動で押す**(enqueueはしない=二重処理なし)。**貼った本人のセッションは、表示されたmsg=IDへ既読印を即押し、作業開始時に着手印を押す**(ミラーにも4状態の可視化を通す)。
