...
- **★★★部門一次対応(dispatch-first)が最終形 (2026-07-16 Chami明示・INC-84)**: Chamiの目標=「**部門chに投げたら、まずその部門のセッションが動く。重大なことだけ研究室へ上がる**」。研究室がmain箱で部門宛てを拾った時の行動:
  ...
  3b. **★起床の正順(INC-85・2026-07-17改訂・全部門必須)**: 起きたら **①mvで自箱を先に退避(`local/inbox/<dept>.jsonl`→`<dept>_work.jsonl`) → ②即waiterを再武装 → ③workファイルを処理→processedへ**。旧順(読む→処理→最後に再武装)は、**waiterの脈が新着到達と同時に止まる**仕様のため、90秒を超える処理中にsweepへ箱ごと奪われる(QA実測2026-07-17・研究室の代打67%の主因)。sweepは空箱を触らない=mv先行で構造的に解決する(INC-76のmv先行ルールの部門箱適用)
