...
- **学習-質問の返信様式(2026-07-13再改定=Chami案)**: 色付きEmbedを復活し、**本文は太字デフォルト**(persona_send側で自動・--nobold で解除)+**要点は見出し(etitle=大きく表示される)へ**。これで色と読みやすさを両立。絵文字プレフィックスは不要に。報告系のP0-P3色分けは従来通り
