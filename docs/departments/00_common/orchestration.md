...
半角括弧 / 秘密を出力・コミットしない / ?v=一括バンプ / Workerデプロイは要承認(★GASは承認不要=Chami裁定2026-07-12) / KV dedup維持 / 成約は観測不可(追わない) / 品質優先6段原則(冒頭) / 改善は承認制(観測→仮説→提案→Chami承認→実装→効果測定) / **戦略・事業文書はGitHubに上げない**(Chami指定 2026-07-12: リポジトリに上げるのはプログラム等の必要物のみ。戦略・目標・優先度は `local/`〔gitignore済〕へ。AIはローカルを直接読むため公開不要) / **★Google Driveは許可されたクラウド(Chami許可2026-07-16「他のクラウドとは違って特別な場所」)**: `local/`の記録(機微=dreams/past/健康を含む)を**Driveへバックアップしてよい**。
  - **経路の条件は"性質"で縛る(実装名では縛らない)**: **非破壊であること=新規作成のみ・削除も上書きもしない**。drive-worker系(削除/上書きAPIが不在)・**G:直コピー(Google Drive for Desktop + robocopy `/E`・`/MIR`不使用・日付別スナップショット)** のいずれも可。※2026-07-17訂正: 当初「drive-worker系に限る」と書いたが、**実装名で縛ると、より安全な実装(Worker非経由=KV制限に触れず・Worker障害と独立・誤削除がクラウドへ伝播しない)が規約違反になる**とデータ整理が指摘。性質での規定へ改めた
