...
- **部門キャラの発言は原則、自部門のチャンネルから**(改修部→#改修-依頼、QA→#品質-QA…)。研究室チャンネルで喋ってよいのはアメスとシャビ・アロンソのみ(「乙女の秘密の部屋に勝手に入室禁止」)
- 研究室で受けた案件を部門が説明する時: 部門chで発言し、研究室にはアメスが「◯◯部が説明してるわ」と橋渡しする
- **受付箱の処理手順(INC-76対策)**: 読む前に `mv discord_inbox.jsonl 一時ファイル` で先に退避し、退避ファイルの全行を処理してからprocessedへ追記する(読了後の追記巻き込みを根絶)
- **学習-質問の返信様式(2026-07-13 Chami指定)**: 話者(コーチ)のテーマカラーEmbedで送る=`persona_send.py --color auto`。色の正本=local/persona_colors.json(オフィスのキャラ色と同一)。他チャンネルは従来通り(報告系は優先度色・通常会話はプレーン)
