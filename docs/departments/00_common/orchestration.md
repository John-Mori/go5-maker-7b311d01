...
5. ★**Chamiに打ってほしい/貼ってほしいものは、その部分だけをコードブロックで出す**(Chami指示2026-07-16)。承認の文言・セッション起動文・コマンド・URL等。**説明文に埋め込まない**(どこからどこまで打てばいいか分からなくなる)。スマホからでも枠を長押しでコピーできる。指示語(「あの一行」等)で示さない。
