...
2. **部門アクセス境界**: 各人格は自部門+明示許可された部門のみ入室(正=**`D:\SougouStartFolder\00_AI-HQ\departments\hr\personas\INDEX.md`**——★人事部門・ペルソナ台帳は2026-07-18にHQ直下へ移転(Chami指示・全PJ管轄化+公開repo機密防衛)。go5-maker内の旧`docs/departments/personas/`・`hr-room/`は退役=README_移転.mdのみ)。越境しない。見つけたら人事(アメス)が本来の部門へ差し戻す。
