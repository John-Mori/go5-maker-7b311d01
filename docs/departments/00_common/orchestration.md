...
- **学習-質問の返信様式(2026-07-13改定)**: Embedは文字が小さく見にくい(Chamiフィードバック)ため会話では廃止。代わりに**冒頭に話者の色絵文字+通常メッセージ**(シーナ🟣/五月🔴/琴葉🟢/莉波🩷)。テーマカラー自体は有効(オフィス・将来のキャラ絵で使用)。**Embedの色分けは報告・通知系(P0-P3・完了報告)専用に残す**
