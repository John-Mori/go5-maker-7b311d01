...
- **★★★部門一次対応(dispatch-first)が最終形 (2026-07-16 Chami明示・INC-84)**: Chamiの目標=「**部門chに投げたら、まずその部門のセッションが動く。重大なことだけ研究室へ上がる**」。研究室がmain箱で部門宛てを拾った時の行動:
  ...
  3d. ★★**ドレインしたら記帳するまでが退避(INC-100・2026-07-18)**: main箱・部門箱を空けたら、**同じ手で処理済み台帳(`local/discord_processed.jsonl`)へ記帳する**まで手を止めない。箱から消えた=処理された、ではない——深夜にmain箱をドレインした誰かの手の中で、Chamiのアイコン差分2便(5枚)が処理も記帳もされず3.5時間消えた実例。恒久対策は台帳ステートマシン化(α発注3)。
