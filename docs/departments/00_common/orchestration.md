...
- **★★★部門一次対応(dispatch-first)が最終形 (2026-07-16 Chami明示・INC-84)**: Chamiの目標=「**部門chに投げたら、まずその部門のセッションが動く。重大なことだけ研究室へ上がる**」。研究室がmain箱で部門宛てを拾った時の行動:
  ...
  3b. **★起床の正順(INC-85 → INC-86で退避先を訂正・全部門必須)**: 起きたら **①mvで自箱を先に退避(`local/inbox/<dept>.jsonl` → `local/_work/<dept>.jsonl`) → ②即waiterを再武装 → ③workファイルを処理→processedへ**。
