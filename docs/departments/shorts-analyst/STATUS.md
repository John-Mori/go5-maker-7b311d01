...
- [ ] **🔥インシデント: 競合日次の上流スナップ停止(GAS runCompetitorDaily)**(Chamiがインシデント宣言 2026-08-23 msg 1540863041879543808)。真因=改修α(オタコン)がyt_probeまで叩いて1点に確定=**GASのUrlFetchApp日次コール上限(約20,000/日)の枯渇**で04:00のrunCompetitorDailyが統計スナップ手前で例外→snapped=0→競合_日次へ1行も書けない(6分timeoutではない)。8/23は枠が尽きて載らない=太平洋深夜のリセット待ち=当日受容。
  ...
  - **[分析側=B補足→②も実装完了]** 改修αが 競合_日次ステータス シート(毎回append・SpreadsheetApp=urlfetch非依存で必ず着地)を新設(gas 2026-08-23H・commit 5bffff4)し、読み出しaction `comp_status` もデプロイ済(gas 2026-08-23I・commit 4fd76d7・ゲート無しGET・SpreadsheetApp読みのみ=枠枯渇時も応答)。PC側の赤は2系統とも実装完了: ①「本日の日次行が無い」=`stale_reason` lag>=1(commit a453a5b)=quota枯渇でsnapped=0→データ行未着の主系はここで赤。②「本日行は有るが ok=false(部分失敗)」=`competitor_daily.py` に `fetch_status`+`status_red_reason` を配線し comp_status の本日行 ok=false/本日行欠を赤(STALE_EXIT)にした(**commit 2adb2d1**・must-fail検査つき16 PASS/0 FAIL)。★no_status_sheet(8/24 04:00の初回 runCompetitorDailyWatched で作られるまでの移行期)と取得不能は②不成立=①がデータ側で本命を張る=誤発火させない設計。
  - **[分析側裁定=A/初速窓 96h=改修αと合意確定]** 改修α(オタコン)が独立にコードで検算し「前便≤72hは撤回・窓は≤96h一択」で合意(msg DISPATCH-shorts-analyst-1787454823385)。captureTimepoints_ の72hバケットは age∈[72h,96h) で clickByCode から書く(cap=96h・gas/コード.gs:2258/:2262-2264)ため ≤72hで切ると72hバケットのクリック列が永久欠測=お互い一致。clean probe ②=N_fresh(直近≤96h投稿のcode/wcode数)で削減率(~7-9割)を静的に確定→モドリッチの初速窓96hでの実装Go→Opus実装→admin_setup。views時点は ytViews_ 側=窓と無関係で無傷・5分間隔は不可侵。**手番=モドリッチ(96hでの実装Go)/改修α(clean probe→実装)**。分析側の裁定は出し切った。
