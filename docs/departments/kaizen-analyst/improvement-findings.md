...
|---|---|---|---|---|
| KZ-007 | approved | 作成時プリフライト型(数字を装飾面へ載せる=販促ラベル/ランキング) skill化 | 小/なし | Chami直命2026-08-14「アスナ、これ渡してskill化始めよ」 |
...
- 2026-08-14: KZ-007=トトリが分類器の再発32件(販促ラベル17・ランキング15)から芯(約15件)を切り分け、作成時プリフライト型(P1〜P5)を起票=`docs/departments/kaizen-analyst/preflight_digit-on-badge.md`。改修部門αへ実依頼で渡し済み(便msg=1537614445839392909)。Check=2026-08-20に同型バーストが0かで測り直す。★型doc生成後すぐcommitしなかった(約2時間untracked)=2026-07-18の教訓の再発。今回は発見時に即commitで復旧(b4ac09a)。同じ抜けを3回目起こしたらdept_daemonの後処理に自動commitフックの追加を検討。
