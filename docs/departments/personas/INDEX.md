...
> **メンバーは3部屋共通= ケヴィン・デ・ブライネ / オタコン(2026-07-16 Chami配置) / 花海咲季 / アメス(補佐)**。
> **★両リーダー制(Chami指定2026-07-16)**: **デ・ブライネ と オタコン の2人がリーダー**。オタコンはシステム開発・機械開発のプロとして設計・実装に参加する(qa-reviewer/report-notifyとの兼任)。役割の目安=デ・ブライネ(判断・実装の推進・Chamiへの直言) / オタコン(解析・信頼性・仕組みの検証)。咲季は実装と報告、アメスは補佐。
| 部屋 | dept | ID | 用途 |
|---|---|---|---|
