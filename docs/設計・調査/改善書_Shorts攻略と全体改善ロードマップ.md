...
4. **前景のフェードイン演出はコンサル指定の演出であり不変**（2026-07-05 Chami確定）。演出を変えて独自路線には行かない → **S-1bは恒久不採用**。動画の見た目に関わる変更は今後もコンサル手法を正とする。
5. **音源付け忘れリマインド（S-2'）も不要**（Chami判断・運用で管理できている）→ S-2'不採用。音源名の記録列も見送り（将来「どの音源が伸びるか」を分析したくなったら再検討）。
...
2. **前景のフェードイン演出はコンサル指定＝不変**: 一般論では「t=0で商品が写っていない」はフック上不利だが、これは**実証済み手法の指定演出**（`app.js:12` REVEAL_START=0.5/REVEAL_DUR=2.0）。変更しない（S-1b恒久不採用・§1前提4）。
...
### ~~S-1b. 冒頭フェードの見直し~~ → ❌ 恒久不採用（2026-07-05 Chami確定）
フェードイン演出は**コンサル指定の演出であり不変**。演出を変えて独自路線には行かない。本タスクおよび派生（フェード短縮・トグル化等）は今後提案しない。

### ~~S-2. BGM焼き込み~~ → ❌ 不採用（音源はYouTubeアプリ側で付与）
音源はYouTubeアプリのShortsエディタで「許可された音楽」を付けるのが主流かつ**ライセンス上唯一の合法経路**（トレンド音源が使えるのはアプリ内追加のみ）。動画ファイルへのBGM焼き込み機能は作らない（P0-4）。

### ~~S-2'. 音源リマインド＋音源名記録~~ → ❌ 不採用（2026-07-05 Chami判断）
付け忘れは運用で管理できているためリマインド不要。音源名の記録列も見送り（将来「どの音源が伸びるか」の分析が欲しくなったら再検討）。

### S-3. メタデータ・運用整備（コード小・運用中心）
- ハッシュタグは**3〜5個**（#shorts＋ジャンル語＋作品固有）。※「15個超で全無効」は二次情報由来の誤りと判明（2026-07-05裏取り）：公式仕様は**60個超で全タグ無視**・タイトル上に表示されるのは上位3個のみ（https://support.google.com/youtube/answer/6390658）。実務推奨3〜5個は変わらず。YT題名生成（`buildTitle` bluesky.js:1460-1473）とYT説明欄テンプレに反映。
...
- **D-1. 取りこぼし回収パック（GAS数行＋クライアント小改修・即効）**
  ...
  4. ~~「音源名」列~~ → 見送り（S-2'不採用に伴い。将来の再検討事項）
...
| ~~S-1b~~ | ~~冒頭フェード見直し~~ **恒久不採用**（コンサル指定演出） | — | — | — | — |
| ~~S-2'~~ | ~~音源リマインド＋音源名記録~~ **不採用**（運用で管理） | — | — | — | — |
| D-1 | 記録の取りこぼし回収パック | gas/コード.gs, bluesky.js | hashtags/rebuildOf/予約メタが記録される | **Opus** | なし |
...
- **施策別の見る指標**: S-1a（背景ループ化）→swipe率(週1目視)と引き込み率／S-3（タグ・時刻）→フィード表示回数(目視)。
...
| リスク | 手当 |
|---|---|
| 演出への意図しない変更（コンサル手法逸脱） | **前景の見た目・演出に触るタスクは提案自体を禁止**（§1前提4）。S-1aも背景素材差し替えのみで演出コードに触れない |
