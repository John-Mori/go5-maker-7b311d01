...
| 2 | 尺の方針 | **5秒維持**（ABなし・ループ数で勝負） | **S-4（尺拡張AB）は不採用**。U-5（WebCodecs）は「フレーム厳密ループ＋バッチ生成」目的のみに縮小＝優先度低。P0-5（FANZA複数コマ許諾）は不要に（1コマ利用の規約確認は引き続き推奨）。**S-1（完全ループ化）の重要度がさらに上がる**（5秒はループ数が生命線） |
