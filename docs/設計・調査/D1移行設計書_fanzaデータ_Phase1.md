...
- [ ] **Phase 1-D(次段)**: 数日dualで監視(D1に実書き込みが溜まりKVと一致し続けるか)後、`USE_D1="on"`でカットオーバー(KV書き込み停止=天井消滅) ← **要承認**。事前にKV↔D1の全件照合をする
- [ ] **Phase 1-E**: KV経路コード削除・KVネームスペース破棄検討

**現状=dual稼働中**。読みはKV(実績経路)、書きはKV+D1両方でD1を実データで検証中。`USE_D1`を`off`に戻して再デプロイすれば即ロールバック。`on`へ進む前にKV↔D1照合で安全確認する。
- 注意(既知・無害): デプロイ直後の数十秒はエッジ伝播ラグで一部PoPが旧版のまま(旧版の挙動で処理)。バグではない。切替直後の1〜2分は挙動が混在しうる。
