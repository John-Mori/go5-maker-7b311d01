...
- [ ] **Phase 1-B(本体) 次段**: 既存ハンドラをストレージ抽象層に載せ替え、`USE_D1="dual"`でデプロイ→数日監視 ← **要承認(worker deploy)**
- [ ] **Phase 1-D**: `USE_D1="on"`カットオーバー(KV書き込み停止=天井消滅) ← **要承認**
- [ ] **Phase 1-E**: KV経路コード削除・KVネームスペース破棄検討

現状: **D1に完全な複製が存在し、いつでも "dual" へ切替可能**。切替前後は `USE_D1` の値を変えて再デプロイするだけ(コードは両対応で待機)。問題時は "off" へ戻せば即ロールバック。
