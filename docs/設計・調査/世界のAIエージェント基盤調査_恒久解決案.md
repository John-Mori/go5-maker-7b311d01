...
策定: 2026-07-17 / データ整理部門(田中琴葉) / 状態: **調査完了・提案確定(実装はChami承認待ち)**
...
| 順 | 恒久解 | 消える問題 | 規模感 | 前提・タイミング |
|---|---|---|---|---|
| 1 | C1 書き込み時検証 | P3-2 | 極小(1関数+順次適用) | いつでも。最初の一手 |
| 2 | A SQLiteメールボックス | P1全部 | 小(約100行+移行) | 事故が現に反復中。デュアルライトで安全に |
| 3 | B worktree分離 | P2全部 | 中(運用再設計) | **改名②下準備と同じ枠**。単独先行しない |
| 4 | C2 ローテーション | P3-1 | 小 | Aの後(台帳がSQLiteに寄ってから) |
| 5 | D 反省バッチ(mem0型) | P4-2/P4-3 | 小〜中 | L2採点運用が回り始めてから |
| 6 | D RAG化 | P4-1 | 中 | 知識パックが上限を恒常的に叩いてから |
