...
> ## ★ このフォルダが「最新版（現行ソース）」です
> 5秒動画メーカー スマホ版の**現行コードはこのフォルダがすべて**です（旧スナップショットは統合・削除済み）。
> 開発の詳しいコンテキスト・座標系の規約・変更履歴・続け方は **[`CLAUDE.md`](CLAUDE.md)** を参照（Claude Code は自動で読み込みます）。
> 公開URL： https://john-mori.github.io/go5-maker-7b311d01/ ／ 最新の構成＝**位置調整スライダー7本**（全体1＋段別の文字3＋帯3）。

iPhone（やAndroid）の**ブラウザだけ**で、写真とテキストから5秒・縦型(9:16)の動画を作るツールです。
**PCもサーバーも不要**。合成は端末内で行われ、写真や動画はどこにもアップロードされません。

このフォルダの中身を**そのままGitHub Pagesに公開**するだけで使えます（現に上記URLで公開中）。
