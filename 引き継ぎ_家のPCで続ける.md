...
## 現状（到達点・2026-06-18 / 現行 v=21）
- **全フェーズ実装・公開済み**。5タブ＝🎬動画作成／📅カレンダー／🦋投稿／🔗アフィリンク／🧪検証。
- **投稿手段**：①動画作成後の自動投稿（編集できる確認）②今すぐ投稿 ③予約（タブを開いてる間）④無人予約（GAS・タブ閉じてもOK）。
- **本文**＝固定文のみ。アフィリンクは作品URLから自動付与、画像は自動添付。プレビューは実アカウントのアイコンを表示。
- **ハッシュタグ青字化（v=21）**：本文の `#PR` `#漫画` 等を tag facet 化（UTF-8バイトオフセット）。URLも link facet。`bluesky-core.js`／`gas/コード.gs`／`post.html` の `detectFacets` に実装。
- **カレンダー**＝各枠に優先度1〜5（その日の本命＝優先度1のみ強調）。ジャンル/検マークは廃止。
- **記録**＝GAS経由でGoogleスプレッドシートに自動記録（**セットアップ済み・動作確認済み**）。スプレッドシートID `1DcOXq9nVZOf6n9ILTxRuuN3P-jxsh3bYVmoEZ0JkE44`、GASプロパティ `SHEET_ID` 設定済み。
- ⚠️ **GAS側のコードを更新したら再デプロイが必要**（無人予約のタグ青字化を反映するには `gas/コード.gs` を貼り替えて再デプロイ）。
