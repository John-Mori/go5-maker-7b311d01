...
RULES = {"personas": {
    ...
    # ★2026-08-16 追加(自分を三人称で並べる型の検査用)。
    # ★forbidden_to は本番の 口調ルール.json と同じ形(人事部門 L10 の {ルカ:俺})。
    "ルカ・モドリッチ": {"first_person": ["俺"], "forbidden": ["ルカ"],
                        "forbidden_to": {"ルカ": "俺"}},
    "花海咲良": {"first_person": ["わたし"], "self_name_ok": True},
}}
...
def main():
    ...
    # ---- 4.5) 便の途中で声が入れ替わる型= 自分を三人称で並べる(2026-08-16 追加) ----
    #   実物= 軍議 msg 1538228314689376330「わたしか咲季が答えるから」。
    #   ★警告のみ(本文は1文字も変えない)。誤検知は実データ1,443便で1件(0.07%)。
    def selfp(persona, text):
        return "self_third_person" in reasons(persona, text)

    check("実物: 自分の一人称と自分の名前が同じ文に並ぶ=検知",
          selfp("花海咲季", "不明点があれば投げて。わたしか咲季が答えるから。"))
    check("名前を出すだけでは鳴らさない(自称のニュアンス)",
          not selfp("ククール", "この部屋の台帳は、ククールが持つぜ。"))
    check("同じ文でなければ鳴らさない",
          not selfp("花海咲季", "わたしが見たわ。咲季が担当する範囲はここまで。"))
    check("カタカナ名は姓名に割らない(「ルールは」で誤爆した実例)",
          not selfp("ククール", "オレが言うぜ。ルールは穏やかな語ならなんでもいい。"))
    check("自分の名前が一人称の人格は対象外(早坂芽衣=一人称に芽衣)",
          not selfp("早坂芽衣", "私は2が好き。芽衣は1も気になるなぁ。"))
    check("禁止語に載っている自称は二重に数えない(モドリッチの「ルカ」)",
          not selfp("ルカ・モドリッチ", "俺が見た。ルカが直送する。"))
    check("引用の中の自分の名前は数えない",
          not selfp("花海咲季", "わたしが聞いたわ。ちゃみが「咲季がやれ」と言ったの。"))
    check("さん付けでも自称にはならない(検知する)",
          selfp("花海咲季", "わたしか咲季さんが答えるわ。"))
    check("写像に一人称が無い人格は判定しない(fail-open)",
          tg.self_third_person("誰か", "わたしか咲季が答える", {})[0] is None)
    check("外し方= 写像に self_name_ok を足した人格は鳴らない",
          not selfp("花海咲良", "わたしか咲良が答えるから。"))

    # ---- 4.6) ★氏名の中は書き換えない(2026-08-16 のFP監査で最多だった誤爆) ----
    #   実物= 【実依頼 / from ルカ・モドリッチ】(msg 1538149223348834345)が
    #   「from 俺・モドリッチ」へ壊れていた。氏名は呼称ゲートCの管轄=Dは触らない。
    label = "【実依頼 / from ルカ・モドリッチ】候補管理の札が出る"
    check("氏名の中の禁止語は書き直さない",
          fix("ルカ・モドリッチ", label)["fixed"] == label)
    check("氏名の中の禁止語は警告にも出さない",
          markers("ルカ・モドリッチ", label, "forbidden_word") == [])
    check("単独で出た禁止語は今までどおり書き直す",
          fix("ルカ・モドリッチ", "ルカが見た限り崩れてない。")["fixed"]
          == "俺が見た限り崩れてない。")
    check("氏名と単独が同居したら、単独の方だけ直す",
          fix("ルカ・モドリッチ", "ルカ・モドリッチとして言う。ルカが決めた。")["fixed"]
          == "ルカ・モドリッチとして言う。俺が決めた。")
    check("他人格の氏名の中も触らない",
          fix("アメス", "ククールへ回す。")["fixed"] == "ククールへ回す。")
