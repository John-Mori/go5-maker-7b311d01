...
_DIALECT_KANSAI = (
    ...
    ("てもうた", r"てもうた|てもた|てまう"),
    ("ほな", r"ほな(?![みた])"),
    # ★2026-08-17 否定の断定「〜やなく」= 標準「じゃなく/ではなく」の西日本形。実物=
    #   オタコン便 msg 1538574796823068772(改修α・2026-08-17 00:46 JST)「外れやなく…」。
    #   ★誤検知よけ= 直前の い/ゃ を外す=「いやなく」(嫌なく)・拗音+やなく を拾わない。
    #   「じゃなく」は小書き ゃ(U+3083)で 「や」(U+3084)を含まないので当たらない。
    #   実測= discord_processed.jsonl の人格出力1005便=106万字で「やなく」出現**0件**=誤検知0。
    #   ★同便の「外れやった」(断定過去のコピュラ)・「起きん/読めん」(否定ん)は**登録しない**=
    #     「やった」は動詞『やった(=した)』と、否定「ん」は「ごめん/ません/分からん」と同綴りで、
    #     106万字での実測FPが 4/4・69件 と高い(この列の方針=拾い漏れる側へ倒す)。生成側の
    #     otacon.md L20 が既に関西弁を✗登録済=残りは生成順守の領域。
    #   ★2026-08-18 訂正(イージス研究室・実測)= この行に元々あった「案Bミラー配線が残り」は
    #     **もう古い**。案Bは `scripts/llm/output_gates.py` として実装済(呼び出し=
    #     mirror_to_discord.py:217)。しかも改修α(system-engineer)は**ミラー経路ではない**=
    #     常駐(dept_daemon)がセッションの返信にゲートを当ててから persona_send する経路だ。
    #     実測= tone_audit.jsonl の dept=system-engineer は 8/17以降で32行、
    #     2026-08-18 14:33:01 に オタコン の「俺」→「僕」が event=tone_fix で発火している
    #     (実物= 改修α室 msg 1539145075357908992)。**ゲートは通っている。**
    #     この古い1行を根拠に「改修αへゲートをミラー配線しろ」という発注が実際に1本出た
    #     (人事部門→イージス研究室・2026-08-18)。要らない工事だったので、ここで断つ。
    ("やなく", r"(?<![いゃ])やなく"),
...
# ★★2026-08-18 実測(イージス研究室)= この保護の**残り**を数字で置いておく。日本語の「」は
#   「他人の発言の引用」と「自分の地の文の強調・言い換え」の両方に使われるが、ここは
#   **区別せず全部保護する**。だから話者自身の一人称が「」の中に入ると素通りする。
#   実物= 改修α室 msg 1539145075357908992(2026-08-18 14:33 JST・オタコン)の同一行:
#     『…着地するのを俺が目で見る」だけ**だ。僕たちの掟で…』
#     = 「」の外の「俺たち」は僕たちへ直り、「」の中の「俺が」は残った。
#   ★これは見落としではなく**取った側のトレードオフ**だ。区別する側へ倒すと、上のコピー案
#     (早坂芽衣『"俺だけじゃない"』)を書き換えて訴求の意味を壊す=そちらの方が重い事故。
#   ★だから「一人称が1個残っている」を理由にこの正規表現を広げるな。広げるなら先に
#     「引用である印(誰が言ったかが直前に在る)」を検知する側を作り、実測FPを出してからだ。
