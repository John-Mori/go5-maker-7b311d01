...
★なぜ要るか(2026-08-23・研究室HQ msg 1540949449638019122 の■3=イージス研究室へ正式発注)
  ...
  **同じ1トークンでも 書込は重み1.25〜2.0・読込は0.10=12.5〜20倍高い。**だから書込側から入る。
  (★HQの当時の本文は「1.25」= 1時間TTLの2.0が判明する前の値。下の ★★2026-08-23決着 を見ろ)
...
★★そして本命は「量」ではなく**キャッシュの寿命の種別**だった(下の --by ttl)。
  ...
  ★**1時間TTLの書込は5分TTLより高い**(5分=基本の1.25倍 / **1時間=2.0倍**)。
  ★★**2026-08-23 決着= 2.0で正しい**(研究室HQ msg 1540974350445060156 が公表価格表を取って確認・
    `docs.claude.com/en/docs/build-with-claude/prompt-caching`。どのモデルも比は同じ)。
    → `quota_burn` 側も直った= `W_CC_1H = 2.0` を新設し、**書込は寿命の種別で別倍率**で数える
      (commit `ab83bd6`。`--cc1h 1.25` で旧来の一律の数え方へ戻して比較できる)。
    → この計器も **q.W_CC_1H / q.W_CACHE_CREATE を読む**= 倍率をここに持たない(正本は quota_burn 1本)。
  ★★同時にHQ側の `MODEL_W` の2つの誤りも直った= **opus 5.0→2.5**(倍に盛っていた) / haiku 0.27→0.5。
    = **Opus便をSonnetへ落とした時の節約は5倍ではなく2.5倍**。②の値付けに直に効く。
  ★俺が測った値は「97.8%が1時間TTL」の方だ。**そこは実測**。
  ★重み付けの結果= **書込が読込の1.53倍重い / 順位が入れ替わる1時間TTLの倍率は1.30**
    = 1.25(旧)と2.0(実価格)の間に境界が在った=「先に書込から入れ」は値段でも裏が取れた。
...
def report_ttl(rows):
    ...
    # ★倍率の正本は quota_burn 1本(ここに数字を持たない)。2026-08-23に公表価格で確定=
    #   5分 1.25 / 1時間 2.0(研究室HQ msg 1540974350445060156・commit ab83bd6)。
    w1, w5 = q.CC1H_W, q.W_CACHE_CREATE
    print("  重み付き書込 %d*%.2f + %d*%.2f = %d" % (h1, w1, h5, w5, int(h1 * w1 + h5 * w5)))
    old = int(tot * q.W_CACHE_CREATE)
    print("    旧(一律%.2f)なら %d = **%.1f%% 過小**に見ていた"
          % (q.W_CACHE_CREATE, old, pct(int(h1 * w1 + h5 * w5) - old, old)))
...
def main():
    ...
    # ★書込は寿命の種別で単価が違う(5分1.25 / 1時間2.0)= 一律で掛けると過小に出る。
    #   collect の行は末尾(添字7)に1時間TTLぶんを持つ(古い行は0=全部5分TTL扱い)。
    c1 = sum((r[7] if len(r) > 7 else 0) for r in rows)
    c5 = cc - c1
    wcc = int(c1 * q.CC1H_W + c5 * q.W_CACHE_CREATE)
    print("  キャッシュ書込 %d (1時間%.2f×%d + 5分%.2f×%d → %d) / 読込 %d (重み%.2f → %d)"
          % (cc, q.CC1H_W, c1, q.W_CACHE_CREATE, c5, wcc,
